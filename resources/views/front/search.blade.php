    ...
    <body class="font-poppins text-[#0E0140] pb-[100px] overflow-x-hidden">
        ...
        <section id="Result" class="container max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px] w-fit">
            ...
            <div class="categories-container grid grid-cols-3 gap-[30px]">
                @forelse ($jobs as $job)
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('front.details', $job) }}"
                ...
                @empty
                    <p class="text-white text-lg">Data belum tersedia</p>
                @endforelse
            ...
            <div id="Pagination" class="flex items-center justify-center gap-[14px] mt-10">
                @if ($jobs->onFirstPage())
                    <button disabled
                        class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold bg-gray-500">«</button>
                @else
                    <a href="{{ $jobs->previousPageUrl() }}&keyword={{ $keyword }}"
                        class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold hover:bg-[#7521FF] transition-all duration-300 bg-[#0E0140]">«</a>
                @endif

                @foreach ($jobs->getUrlRange(1, $jobs->lastPage()) as $page => $url)
                    @if ($page == $jobs->currentPage())
                        <button
                            class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold bg-[#7521FF]">{{ $page }}</button>
                    @else
                        <a href="{{ $url }}&keyword={{ $keyword }}"
                            class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold hover:bg-[#7521FF] transition-all duration-300 bg-[#0E0140]">{{ $page }}</a>
                    @endif
                @endforeach

                @if ($jobs->hasMorePages())
                    <a href="{{ $jobs->nextPageUrl() }}&keyword={{ $keyword }}"
                        class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold hover:bg-[#7521FF] transition-all duration-300 bg-[#0E0140]">»</a>
                @else
                    <button disabled
                        class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center text-white font-semibold bg-gray-500">»</button>
